...
csv_path = 'biosafety_9-10/'  # 数据文件路径
csv_file = '2001_2020.csv'  # 数据文件名
...
def embedding(word_list, word_need):
    ...
    word_vec_matrix = [[fasttext_model[word] for word in words] for words in word_need]
...
def pretreatment():
    try:
        with open(word_list_path + word_list_file, 'r') as f:
            final_text = eval(f.read())
        print('读取word_text成功')
        filtered_words = [[]]  # 若保存有分词结果，则默认也保存有词向量，不再提供词向量语料！
    except:
        csv_name = csv_path + csv_file
        df = pd.read_csv(csv_name, sep='|')
        print('数据量:'+str(df.shape[0]))
        input_data = []
        for i in range(df.shape[0]):
            input_data.append(str(df['title'][i])+' '+str(df['abstract'][i]))  # 读取为list，元素为title和abstract
        stopwords_list = list(set(stopwords.words('english')))  # 加载停用词
        input_data_low = [text.lower() for text in input_data]  # 转小写
        word_list = [word_tokenize(text) for text in input_data_low]  # 分词
        english_punctuations = [
            ',', '+','-','±','.', ':','|', ';', '-','"',"'",'?', '(','/i', ')', '[', ']', '&', '!', '*', '@', '#', '$', '%','’','≤','a.','b.','c.','d.','e.','m.', 'n.', 'p.', 'f.','g.','h.','i.','j.','k.','l.','o.','q.','r.','s.','t.','u.','v.','w.','x.','y.','z.','=','<','>'
        ]  # 标点符号
        filtered_words = [[word for word in text if word not in english_punctuations] for text in word_list]  # 过滤标点
        word_len1 = [[word for word in text if len(word) > 1] for text in word_list]
        text_list = [[w for w in text if w not in stopwords_list] for text in word_len1]  # 过滤停用词
        dropword = [
            '/sup', '/sub', 'sub', 'sup', '/mn', '/mo', '/i', "''", "'s"
        ]  # 主题无关的词
        text_list_drop = [[word for word in text if word not in dropword] for text in text_list]  # 过滤无关词
        final_text_withoutlemmatizer = [[word for word in text if bool(re.search(r'\d', word)) == False] for text in text_list_drop]
        # 过滤数字，得到二维列表
        wordnet_lemmatizer = WordNetLemmatizer()
        final_text = [[wordnet_lemmatizer.lemmatize(word) for word in text] for text in final_text_withoutlemmatizer]
        # 词形还原
        with open(word_list_path + word_list_file, 'w') as f:
            f.write(str(final_text))  # 保存分词结果
        print('文本预处理完成')
        '''
        预处理完成
        '''
    dictionary = Dictionary(final_text)  # 构建gensim字典，包含词编号，词频，词名
    ...
    '''
    word_vec_matrix = embedding(filtered_words, lda_list[0])  # 词向量获取，使用未过滤停用词的语料
    '''
    获取词向量
    '''
